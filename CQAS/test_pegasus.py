...
def test_model_performance(model_path, cpu: bool):
    ...
    if cpu:
        input_ids = tokenizer(input_examples, max_length=512, truncation=True, padding='max_length',
                              return_tensors='pt').to('cpu')
    else:
        input_ids = tokenizer(input_examples, max_length=1024, truncation=True, padding='max_length',
                              return_tensors='pt')
    ...
    t0 = time.time()
    for s in summaries:
        prediction = tokenizer.decode(s, skip_special_tokens=True,
...
test_model_performance("google/pegasus-xsum", True)  # Baseline pegasus
# test_model_performance("output/pegasus/", True)  # Fine-tuned pegasus
