...
parse.add_argument("--dataset", choices=["comparg", "comp_ident"], default="comparg")
...
def load_comparg_data():
    ...
    train_df["labels"] = train_df["labels"].apply(lambda x: x - 1 if x > 0 else 0)
    ...
    test_df["labels"] = test_df["labels"].apply(lambda x: x - 1 if x > 0 else 0)
...
def get_preprocessed_comparg_dataset(
    ...
    def preprocess_function(examples):
        inputs = []
        for obj0, obj1, ans in zip(
            examples["object_0"], examples["object_1"], examples["answer"]
        ):
            inputs.append(
                "Object 1: " + str(obj0) + "; " + "Object 2: " + str(obj1) + "; " + ans
            )
