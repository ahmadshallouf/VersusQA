...
B_SYS, E_SYS = "<<SYS>>\n", "\n<</SYS>>\n\n"
SYSTEM_PROMPT = B_SYS + """You are a helpfull assistnat for sequence labeling with the following labels: OBJ - Object, ASP - Aspect, PRED - Predicate and O - none.
...
def group_to_examples(group):
    input_sentence = " ".join([f"[ID-{id}]{token}" for id, token in enumerate(group['words'].values)])
    target_sentence = " ".join([f"[ID-{id}][{label}]" for id, label in enumerate(group['labels'].values)])
    return input_sentence, target_sentence


def df_to_dataset(df):
    dataset = []
    for group_id, group in tqdm(df.groupby('sentence_id'), total=len(df.sentence_id.unique())):
...
if __name__ == '__main__':
    ...
    ds = datasets.DatasetDict({
        ...
        'validation': df_to_dataset(pd.read_csv(Path(args.data_path) / 'valid.csv')),
